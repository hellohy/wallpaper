...
window.onload = function () {
    ...
    for (var ii = 0; ii < 1200; ii++) {
        ...
        var ball = new Ball(1, 1, new Vector2(window.innerWidth * (Math.random() + Math.random() ) / 2, window.innerHeight * (Math.random() + Math.random()) / 2), vel);
        balls.push(ball)
    }
...
Floor.prototype.getClosestPoints = function (ball, ctx) {
    ...
    var dis = Math.sqrt(window.innerWidth * window.innerWidth + window.innerHeight * window.innerHeight);

    opacity = .02 * (1 - dist / dis);
...
Ball.prototype.changePosAndVel = function () {
    if (this.vel.x > 0) {
        if (this.pos.x > window.innerWidth + 0) {
            ...
            this.pos.y = window.innerHeight * (Math.random() + Math.random()) / 2;
        }

    } else if (this.vel.x < 0) {
        if (this.pos.x < 0) {
            this.pos.x = window.innerWidth + 50 * (Math.random() + Math.random()) / 2;
            this.pos.y = window.innerHeight * Math.random();
        }

    } else if (this.vel.y > 0) {
        if (this.pos.y > window.innerHeight + 50) {
            this.pos.x = window.innerWidth * (Math.random() + Math.random()) / 2;
        ...
        if (this.pos.y < 0) {
            ...
            this.pos.y = window.innerHeight + 50 + 50 * (Math.random() + Math.random()) / 2;
        }
